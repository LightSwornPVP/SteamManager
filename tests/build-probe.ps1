param([string]$AppDirectory=(Join-Path $PSScriptRoot '..\artifacts\preview-003'),[string]$GamePath='C:\Program Files (x86)\Steam\steamapps\common\Valheim',[string]$ProbeName='SteamManager.BehaviorProbe.dll',[switch]$EligibilityOnly)
...
if($EligibilityOnly){$compileArgs+=(Join-Path $PSScriptRoot 'EligibilityProbe.cs')}else{
