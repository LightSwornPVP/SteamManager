...
class BlueprintFileTests
{
    ...
    static int Main()
    {
        ...
        Reject(()=>Parse(Piece.Replace(";1;1;1",";2;1;1")),"Scaled geometry rejected instead of silently changing shape");
        ...
        Reject(()=>Parse(string.Join("\n",Enumerable.Repeat(Piece,501))),"Piece cap enforced");
        Reject(()=>Parse(new string('x',512001)),"Input size cap enforced");
